...
# 🔑 توكن البوت
BOT_TOKEN = "8868110647:AAHjffbopZF-p6E9EP9HqB3a2l99IbG-TeI"
...
def get_db_connection():
    conn = sqlite3.connect("game.db", timeout=20)
...
# ------------------- قواميس الألعاب (20+ عنصر لكل قائمة) -------------------
active_games = {} 
pending_orders = {}
...
# 🏛️ إضافة قاموس لعبة اللهجات العربية الجديدة (20 عنصر)
DIALECTS = {
    "ما معنى كلمة 'زقرت' باللهجة السورية واللبنانية؟": "قبضاي",
    "ما معنى كلمة 'مبحرين' أو 'بحر فيني' باللهجة الحجازية؟": "يطالع",
    "ما معنى كلمة 'هسه' باللهجة العراقية؟": "الان",
    "ما معنى كلمة 'باهي' باللهجة التونسية والليبية؟": "جميل",
    "ما معنى كلمة 'وايد' باللهجة الخليجية؟": "كثير",
    "ما معنى كلمة 'برشا' باللهجة التونسية؟": "كثير",
    "ما معنى كلمة 'ديالي' باللهجة المغربية؟": "حقي",
    "ما معنى كلمة 'جدع' باللهجة المصرية؟": "شجاع",
    "ما معنى كلمة 'دحين' باللهجة السعودية؟": "الان",
    "ما معنى كلمة 'كوشة' باللهجة السودانية؟": "مزبلة",
    "ما معنى كلمة 'شين' باللهجة الخليجية والسودانية؟": "قبيح",
    "ما معنى كلمة 'غاوية' باللهجة العمانية؟": "جميلة",
    "ما معنى كلمة 'بزاف' باللهجة الجزائرية والمغربية؟": "كثير",
    "ما معنى كلمة 'قرت قرة' باللهجة اليمنية؟": "جلست",
    "ما معنى كلمة 'إرمس' باللهجة الإماراتية؟": "تكلم",
    "ما معنى كلمة 'تكة' باللهجة الأردنية والفلسطينية؟": "قليل",
    "ما معنى كلمة 'خاشوقة' باللهجة الإماراتية والعراقية؟": "ملعقة",
    "ما معنى كلمة 'سيباوي' باللهجة المصرية (سيب الحاجه)؟": "اتركها",
    "ما معنى كلمة 'سيده' باللهجة الخليجية (امشي سيده)؟": "مستقيم",
    "ما معنى كلمة 'چندوب' أو 'جنزير' باللهجة العراقية؟": "سلسلة"
}
...
# ------------------- نظام المتجر (تم الإصلاح الشامل هنا للشرطة السفلية) -------------------
...
def handle_buy_click(call):
    ...
    bot.edit_message_text(f"✅ اخترت: **{order_type_map[item_key]}**\n\n🖼️ **الآن:** أرسل صورة علمك/دولتک للطلب.", call.message.chat.id, call.message.message_id)
...
def handle_flag_photo(message):
    ...
    markup = InlineKeyboardMarkup()
    # 🌟 تم التعديل هنا: استخدام (:) كفاصل آمن لحماية الـ Usernames الحاوية على (_)
...
def handle_painter_selection(call):
    user_id = call.from_user.id
    # 🌟 تم التعديل هنا: التقسيم باستخدام الـ (:) لمنع كسر الـ username
    ...
    if not painter_data:
        ...
        return bot.edit_message_text(f"⚠️ **عذراً!** لا يمكن الإرسال للرسام {painter} لأنه لم يقم بالدخول للبوت مسبقاً.\n(يجب عليه الدخول والضغط على /start لتسجيل حسابه).", call.message.chat.id, call.message.message_id)
    ...
    try:
        ...
        bot.edit_message_text(f"✅ **تمت العملية بنجاح!**\nخصمنا {order['price']} نقطة، وتم إرسال طلبك مباشرة لحساب الرسام {painter}.", call.message.chat.id, call.message.message_id)
    except Exception as e:
        bot.edit_message_text(f"⚠️ فشل إرسال الطلب للرسام {painter} (قد يكون قام بحظر البوت).", call.message.chat.id, call.message.message_id)
...
def start_specific_game(message):
    ...
    cmd = message.text.split()[0].split('@')[0].replace('/', '')
    # 🎲 تعديل النطاق العشوائي والتحديد ليشمل 16 لعبة
    ...
    # 🏛️ التحدي الـ 16 الجديد: لعبة اللهجات العربية
